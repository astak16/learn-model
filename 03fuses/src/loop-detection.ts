...
const hash = (input: string) => createHash("sha256").update(input).digest("hex").slice(0, 16);

const hasToolCall = (toolName: string, params: unknown): string => {
  return `${toolName}:${hash(stableStringify(params))}`;
};
...
const getNoProgressStreak = (toolName: string, argsHash: string): number => {
  let streak = 0;
...
const getPingPongCount = (currentHash: string): number => {
...
export const detect = (toolName: string, params: unknown): DetectionResult => {
  const argsHash = hasToolCall(toolName, params);
  ...
  if (noProgress >= BREAKER_THRESHOLD) {
    return {
      ...
      count: noProgress,
      message: `【熔断】${toolName} 已重置 ${noProgress} 次且无进展，强制停止`,
    };
  }
