...
const createDelayedStream = (chunks: LanguageModelV3StreamPart[], delayMs = 30): ReadableStream => {
...
function detectParallelIntent(text: string): ToolCallIntent[] | null {
  if (text.includes("测试并发") || text.includes("test parallel")) {
    return [
      ...
      { toolName: "list_directory", args: { city: "" } },
    ];
  }
  return null;
}

function detectToolIntent(prompt: LanguageModelV3CallOptions["prompt"]) {
  ...
  if (match && text.includes("写")) {
    return { toolName: "write_file", args: { path: match[1], content: match[2] } };
  }
  ...
  if (hasToolResults(prompt)) {
    ...
    for (const tm of toolMsgs) {
      ...
      for (const c of content) {
        if (typeof c !== "string" && c.type === "tool-result") {
          parts.push(stringifyToolOutput(c.output));
        }
      }
    }
  ...
  return {
    ...
    async doGenerate({ prompt }: LanguageModelV3CallOptions) {
      ...
      if (text.includes("测试重试") || text.includes("test retry")) {
        ...
        return {
          content: [{ type: "text" as const, text: "重试成" }],
